...
changes <- left_join(res_ead_vs_wd_mid, res_ead_mid, by = "geneid") %>%
  left_join(., res_wd_mid, by = "geneid") %>%
...
write.table(filtered_list, "target_genes_mid.tsv", sep="\t", row.names=T, col.names=T, quote=F)
...
write.table(unknown_targets, "unknown_targets_mid.tsv", sep="\t", col.names=T, row.names=F, quote=F)

#get list of known genes
known_targets = anti_join(filtered_list, unknown_targets)
write.table(known_targets, "known_targets_mid.tsv", sep="\t", col.names=T, row.names=F, quote=F)
