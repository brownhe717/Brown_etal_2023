...
changes <- left_join(res_ead_vs_wd_early, res_ead_early, by = "geneid") %>%
  left_join(., res_wd_early, by = "geneid") %>%
...
write.table(filtered_list, "target_genes_early.tsv", sep="\t", row.names=T, col.names=T, quote=F)
...
write.table(unknown_targets, "unknown_targets_early.tsv", sep="\t", col.names=T, row.names=F, quote=F)

#get list of known genes
known_targets = anti_join(filtered_list, unknown_targets)
write.table(known_targets, "known_targets_early.tsv", sep="\t", col.names=T, row.names=F, quote=F)
