...
ead_for_heatmap <- res_ead_sig_late %>%
  dplyr::select("geneid")

wd_for_heatmap <- res_wd_sig_late %>%
  dplyr::select("geneid")

heatmap_genes <- bind_rows(ead_for_heatmap, wd_for_heatmap) %>% 
  distinct

ead_log2fc <- filter(res_ead_late, geneid %in% heatmap_genes$geneid) %>%
  dplyr::select("geneid", "log2FoldChange")

wd_log2fc <- filter(res_wd_late, geneid %in% heatmap_genes$geneid) %>%
  dplyr::select("geneid", "log2FoldChange")
...
Heatmap(sig_matrix, col = colorRamp2(c(-10, -5, 0, 5, 10), c("midnightblue", "cadetblue3", "snow1", "chocolate2", "firebrick")),
        ...
        column_labels = c("120hr WT eye-antennal disc", "120hr WT wing disc"), column_names_side = "top", column_title_side = "top", 
        column_title = "144hr Pc knockdown eye-antennal disc expression vs.")
