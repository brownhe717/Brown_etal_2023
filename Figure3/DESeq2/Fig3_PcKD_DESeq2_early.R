...
# Perform DE testing 
res_ead_vs_wd_early <- results(dds, contrast=c("tissue","WT_WD96","WT_EAD96"))

# Add fold change classification to complete results
res_ead_vs_wd_early <- as.data.frame(res_ead_vs_wd_early) %>%
...
res_ead_vs_wd_sig_early <- subset(res_ead_vs_wd_early, padj < 0.05 & abs(log2FoldChange) >= 1)

# Write to table
write.table(res_ead_vs_wd_sig_early, "EAD.v.WD.diff.96.txt", sep="\t", col.names=T, row.names=F, quote=F)
...
# Perform DE testing 
res_ead_early <- results(dds, contrast=c("tissue","PcKD_EAD108", "WT_EAD96"))

# Add fold change classification to complete results
res_ead_early <- as.data.frame(res_ead_early) %>%
  rownames_to_column("geneid") %>%
...
# Pull significantly changed genes
res_ead_sig_early <- subset(res_ead_early, padj < 0.05 & abs(log2FoldChange) >= 1)

# Write to table
write.table(res_ead_sig_early, "PcG108.v.EAD96.diff.txt", sep="\t", col.names=T, row.names=F, quote=F)
...
# Perform DE testing 
res_wd_early <- results(dds, contrast=c("tissue","PcKD_EAD108", "WT_WD96"))

# Add fold change classification to complete results
res_wd_early <- as.data.frame(res_wd_early) %>%
  rownames_to_column("geneid") %>%
...
# Pull significantly changed genes
res_wd_sig_early <- subset(res_wd_early, padj < 0.05 & abs(log2FoldChange) >= 1)

# Write to table
write.table(res_wd_sig_early, "PcG108.v.WD96.diff.txt", sep="\t", col.names=T, row.names=F, quote=F)
