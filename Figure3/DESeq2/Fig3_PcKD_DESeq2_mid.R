...
# Perform DE testing 
res_ead_vs_wd_mid <- results(dds, contrast=c("tissue","WT_WD108","WT_EAD108"))

# Add fold change classification to complete results
res_ead_vs_wd_mid <- as.data.frame(res_ead_vs_wd_mid) %>%
...
res_ead_vs_wd_sig_mid <- subset(res_ead_vs_wd_mid, padj < 0.05 & abs(log2FoldChange) >= 1)

# Write to table
write.table(res_ead_vs_wd_sig_mid, "EAD.v.WD.diff.108.txt", sep="\t", col.names=T, row.names=F, quote=F)
...
# Perform DE testing 
res_ead_mid <- results(dds, contrast=c("tissue","PcKD_EAD120", "WT_EAD108"))

# Add fold change classification to complete results
res_ead_mid <- as.data.frame(res_ead_mid) %>%
  rownames_to_column("geneid") %>%
...
# Pull significantly changed genes
res_ead_sig_mid <- subset(res_ead_mid, padj < 0.05 & abs(log2FoldChange) >= 1)

# Write to table
write.table(res_ead_sig_mid, "PcG120.v.EAD108.diff.txt", sep="\t", col.names=T, row.names=F, quote=F)
...
# Perform DE testing 
res_wd_mid <- results(dds, contrast=c("tissue","PcKD_EAD120", "WT_WD108"))

# Add fold change classification to complete results
res_wd_mid <- as.data.frame(res_wd_mid) %>%
  rownames_to_column("geneid") %>%
...
# Pull significantly changed genes
res_wd_sig_mid <- subset(res_wd_mid, padj < 0.05 & abs(log2FoldChange) >= 1)

# Write to table
write.table(res_wd_sig_mid, "PcG120.v.WD108.diff.txt", sep="\t", col.names=T, row.names=F, quote=F)
